...
class makeGraph:
    ...
    def resetHeads(self):
        self.headsCntr.set(0) 
     
    def resetTails(self):
        self.tailsCntr.set(0)
    ...
    def reset(self,event=None):
        #reset the bars to normal
        
        #delete the rectangles
        self.canvas.delete(self.bRect)
        self.canvas.delete(self.gRect)
        
        #add new rectangles at the starting position
        self.bRect = self.canvas.create_rectangle(150,400,225, 400,fill="#3399FF") #(150,30,225,400)
        self.gRect = self.canvas.create_rectangle(350,400,425, 400,fill="#217C7E") #(350,30,425,400)
        ...
        #delete the old counters label
        self.canvas.delete(self.labelHeadsCntr)
        self.canvas.delete(self.labelTailsCntr)
        
        #recreate the variables for some reason???
        self.headsCntr = IntVar()
        self.tailsCntr = IntVar()
        self.totalCntr = IntVar()
        self.probHeads= DoubleVar()
        ...
        #recreate the counter labels
        self.labelHeadsCntr = self.canvas.create_text(187.5,15,text=self.getHeadsNum(),justify=CENTER,anchor=N)   #initial (187.5,15)
        self.labelTailsCntr = self.canvas.create_text(387.5,15,text=self.getTailsNum(),justify=CENTER,anchor=N)      #initial (387.5,15)
